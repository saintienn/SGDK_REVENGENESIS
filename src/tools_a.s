...
.short_match:                       ;// Short match %110...
    ...
    lsr.b   #1,%d0
    beq   .end_decrunch           ;// if offset == 0, end of decrunching
    bcs.b   .domatch_new_lastpos
    moveq   #2,%d2                  ;// length = 2
    bra   .domatch_new_lastpos

.code_pair:                         ;// Code pair %10...
    bsr   .decode_gamma
    sub.w   %d1,%d2                 ;// offset -= LWM
...
.compare_1280:
    cmp.w   %a4,%d0                 ;// >=1280 <32000
    bge.b   .domatch_with_inc

.compare_128:
    cmp.w   %a5,%d0                 ;// >=128 <1280
...
.copy_code_pair:
    ...
    move.w  %d2,%d5        ;// d5 = length
    subq.w  #1,%d5         ;// dbf precisa de N-1
