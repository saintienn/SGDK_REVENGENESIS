...
public class LITEPACK {
    ...
    public static byte[] pack(byte[] data, int start, boolean silent) throws IOException, IllegalArgumentException {
        ...
        if (len >= 2) {
            ...
            // IMPROVEMENT: store top-K candidates per position for better DP decisions
            final int K_CANDIDATES = 4;
            final Match[][] candidates = new Match[wdata.length - offset][];

            for (int i = 0; i < candidates.length; i++)
                candidates[i] = findTopKMatches(wordMatches, wdata, offset + i, start / 2, K_CANDIDATES);

            // PASS 3: walk backward in matches and find optimal match length (true DP)
            // IMPROVEMENT: consider all K candidates per position, not just the best 1
            final int costs[] = new int[candidates.length + 1];
            final Match[] matches = new Match[candidates.length];

            // initialize ending cost
            costs[candidates.length] = 0;
            ...
            for (int i = candidates.length - 1; i >= 0; i--) {
                ...
                // IMPROVEMENT: try all K candidates and pick the one with lowest total cost
                if (candidates[i] != null) {
                    for (Match match : candidates[i]) {
                        if (match == null) break;
                        int matchCost = match.cost + costs[i + match.length];
                        // strictly less: prefer match over literal when equal cost (better compression)
            ...
            // IMPROVEMENT: lazy evaluation pass
            // If position i is a literal but position i+1 has a match that saves more,
            // try converting i to a match to see if total cost improves.
            for (int i = 0; i < matches.length - 1; i++) {
                if (matches[i] == null && matches[i + 1] != null) {
                    // check if starting a match at i (if any) + cost[i+match.length]
                    // is better than literal[i] + cost[i+1]
                    if (candidates[i] != null) {
                        for (Match m : candidates[i]) {
                            if (m == null) break;
                            int mc = m.cost + costs[i + m.length];
                            if (mc < costs[i]) {
                                costs[i] = mc;
                                matches[i] = m;
                                // ripple forward: update costs from i backward
                                // (simple single pass is sufficient for local improvement)
                                break;
                            }
                        }
                    }
    ...
    private static Match findBestMatchInternal(short[] wdata, int from, int ind, int originStart)
    {
        ...
        // IMPROVEMENT: extend match as far as possible (not just repeat run)
        refOffset = from;
        curOffset = ind;
        len = 0;
        while ((curOffset < wdata.length) && (refOffset < ind) &&
               (wdata[refOffset] == wdata[curOffset]) && (len < maxLen))
        {
            refOffset++;
            curOffset++;
            len++;
    ...
    /**
     * IMPROVEMENT: find top K best matches for position ind.
     * Returns array of up to K matches sorted by savedWord descending.
     * This allows the DP in Pass 3 to make globally optimal decisions.
     */
    private static Match[] findTopKMatches(WordMatchList[] wordMatches, short[] wdata, int ind,
            int originStartOffset, int k)
    {
        final int offMin = Math.max(originStartOffset - MATCH_LONG_OFFSET_MAX, 0);
        final WordMatchList wml = wordMatches[wdata[ind] & 0xFFFF];
        ...
        final java.util.List<Match> allMatches = new java.util.ArrayList<>();
        ...
        while (wmlInd < wml.size())
        {
            ...
            if ((off >= originStartOffset) || ((off + repeat) < originStartOffset))
            {
                if (repeat < curRepeat)
                {
                    final Match match = new Match(ind, off,
                            Math.min(MATCH_LONG_MAX_SIZE, repeat + 1), off < originStartOffset);
                    if (match.savedWord >= 1)
        ...
        // sort by savedWord descending, prefer shorter offset on tie
        allMatches.sort((a, b) -> {
            if (b.savedWord != a.savedWord) return b.savedWord - a.savedWord;
            return a.getRelativeOffset() - b.getRelativeOffset();
        });

        // return top K
        int size = Math.min(k, allMatches.size());
        Match[] result = new Match[size];
        for (int i = 0; i < size; i++)
            result[i] = allMatches.get(i);
